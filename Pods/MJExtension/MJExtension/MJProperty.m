...
- (void)setProperty:(objc_property_t)property
{
    ...
    // 2.成员类型
    NSString *attrs = @(property_getAttributes(property));// 包装成C语言的字符串
    NSUInteger dotLoc = [attrs rangeOfString:@","].location;// 能不能找到
    NSString *code = nil;
    NSUInteger loc = 1;
    if (dotLoc == NSNotFound) { // 没有找到
        code = [attrs substringFromIndex:loc];// 从下表1取
    } else {// 找到了
        code = [attrs substringWithRange:NSMakeRange(loc, dotLoc - loc)];// 找到了
    }
...
{
    if (self.type.KVCDisabled) return [NSNull null];// ??? Iva 是变量 ？ 变量不支持KVC
