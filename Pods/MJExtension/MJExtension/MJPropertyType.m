...
+ (instancetype)cachedTypeWithCode:(NSString *)code
{
    ...
    MJPropertyType *type = types[code];//获取类型
    if (type == nil) {
        type = [[self alloc] init];
        type.code = code;// 类型标识符
        types[code] = type;// 是什么类型
    }
...
- (void)setCode:(NSString *)code
{
    ...
    if ([code isEqualToString:MJPropertyTypeId]) {// id类型
        _idType = YES;
    } else if (code.length == 0) {// 没有类型就不支持KVC吗？？？？
        _KVCDisabled = YES;
