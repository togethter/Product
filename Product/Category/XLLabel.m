...
@property(nonatomic, strong)NSTextStorage *XLTextString;


/* 点击类型 */
@property(nonatomic, assign)XLLabelStyle ClickStyle;
...
-(void)setXLLabelNormalColor:(UIColor *)XLLabelNormalColor{
    _XLLabelNormalColor = XLLabelNormalColor;
    [self prepareText];
}

-(void)setXLLabelMatchArr:(NSArray<NSDictionary *> *)XLLabelMatchArr{
    _XLLabelMatchArr = XLLabelMatchArr;
    [self prepareText];
}
...
-(void)setup{
    ...
    _XLTextString = [NSTextStorage new];
    _layout = [NSLayoutManager new];
    _textContainer = [NSTextContainer new];
    _XLLabelNormalColor = [UIColor colorWithRed:162.0/255 green:162.0/255  blue:162.0/255  alpha:162.0/255];
    _XLLabelHightLightBackgroundColor = [UIColor colorWithWhite:0.7 alpha:0.2];
...
-(void)drawRect:(CGRect)rect{
    ...
    if (self.selectedRange.length != 0) {
        ...
        // 2.0.确定颜色
        UIColor *selectedColor = self.isSelected ? self.XLLabelHightLightBackgroundColor : [UIColor clearColor];
        
        // 2.1.设置颜色
        [self.XLTextString addAttribute:NSBackgroundColorAttributeName value:selectedColor range:self.selectedRange];
    ...
    // 2.绘制字形
    // 需要绘制的范围
    NSRange range = NSMakeRange(0, self.XLTextString.length);
...
-(void)prepareTextSystem{
    ...
    // 1.将布局添加到storeage中
    [self.XLTextString addLayoutManager:self.layout];
...
-(void)prepareText{
    ...
    attr = @{
             ...
             NSForegroundColorAttributeName : self.XLLabelNormalColor
             };
    
    [attrStringM setAttributes:attr range:NSMakeRange(0, attrStringM.length)];
    
    // 4.设置XLTextString的内容
    [self.XLTextString setAttributedString:attrStringM];
    ...
    for (NSValue *value in linkRanges) {
        ...
        [self.XLTextString addAttribute:NSForegroundColorAttributeName value:self.linkHightColor range:range];
    }
    ...
    for (NSValue *value in phoneNumberRanges) {
        NSRange range;
        [value getValue:&range];
        [self.XLTextString addAttribute:NSForegroundColorAttributeName value:self.PhoneNumberHightColor range:range];
    }
    ...
    for (NSValue *value in userRanges) {
        NSRange range;
        [value getValue:&range];
        [self.XLTextString addAttribute:NSForegroundColorAttributeName value:self.userHightColor range:range];
    }
    ...
    for (NSValue *value in topicRanges) {
        NSRange range;
        [value getValue:&range];
        [self.XLTextString addAttribute:NSForegroundColorAttributeName value:self.topicHightColor range:range];
    }
    ...
    for (NSValue *value in agreementRanges) {
        NSRange range;
        [value getValue:&range];
        [self.XLTextString addAttribute:NSForegroundColorAttributeName value:self.agreementHightColor range:range];
    }
    
    // 10.匹配用户自定义的字符串
    if (self.XLLabelMatchArr.count > 0) {
        ...
        if (userDefineRangeDicts.count > 0) {
            ...
            for (NSDictionary *dict in userDefineRangeDicts) {
                NSValue *value = dict[XLRange];
                [arrM addObject:value];
                UIColor *color = dict[XLColor];
                NSRange range;
                [value getValue:&range];
                [self.XLTextString addAttribute:NSForegroundColorAttributeName value:color range:range];
            }
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    // 3.取出内容
    NSString *selectedString = [[self.XLTextString string] substringWithRange:self.selectedRange];
    
    // 3.回调
    switch (self.ClickStyle) {
        case XLLabelStyleAgreement:{
            __weak typeof(self) weakSelf = self;
            if (self.XLTapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStyleAgreement, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XLLabelStyleLink:{
            ...
            if (self.XLTapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStyleLink, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XLLabelStyleTopic:{
            ...
            if (self.XLTapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStyleTopic, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XLLabelStyleUser:{
            ...
            if (self.XLTapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStyleUser, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XLLabelStyleUserDefine:{
            __weak typeof(self) weakSelf = self;
            if (self.XLTapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStyleUserDefine, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XLLabelStylePhoneNumber:{
            ...
            if (self.XLTapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.XLTapOperation) {
                    strongSelf.XLTapOperation(strongSelf, XLLabelStylePhoneNumber, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
    ...
    // 4.代理
    if ([self.delegate respondsToSelector:@selector(XLLabel:didSelectedString:forXLLabelStyle:onRange:)]) {
        
        [self.delegate XLLabel:self didSelectedString:selectedString forXLLabelStyle:self.ClickStyle onRange:self.selectedRange];
    }
...
-(NSRange)getSelectRange:(CGPoint)selectPoint{
    ...
    if (self.XLTextString.length == 0) return NSMakeRange(0, 0);
    ...
    for (NSValue *value in self.linkRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStyleLink;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStyleUser;
            return range;
        }
    }
    ...
    for (NSValue *value in self.topicRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStyleTopic;
            return range;
        }
    }
    ...
    for (NSValue *value in self.agreementRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStyleAgreement;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userDefineRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStyleUserDefine;
            return range;
        }
    }
    // 2.6.判断是否是一个电话号码
    for (NSValue *value in self.PhoneNumberRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.ClickStyle = XLLabelStylePhoneNumber;
            return range;
        }
    }
...
-(NSArray<NSDictionary*> *)getUserDefineStringsRange{
    ...
    if (self.XLLabelMatchArr.count == 0) return nil;
    
    NSMutableArray<NSDictionary*> *arrM = [NSMutableArray array];
    
    NSString *str = [self.XLTextString string];
    for (NSDictionary *dict in self.XLLabelMatchArr) {
        ...
        aDictM[XLRange] = value;
        aDictM[XLColor] = color;
...
-(NSArray *)getRangesFromResult:(NSRegularExpression *)regex{
    ...
    NSArray<NSTextCheckingResult *> *results = [regex matchesInString:[self.XLTextString string] options:0 range:NSMakeRange(0, self.XLTextString.length)];
