...
//@property(nonatomic,strong)  MBProgressHUD *newhud;
...
+(void)addSting:(NSString * )str toView:(UIView *)view{
   ...
   // newhud.delegate = self;
    ...
    //显示的文字
    newhud.labelText = @"Test";
    //细节文字
    newhud.detailsLabelText = @"Test detail";
    //是否有庶罩
...
+ (void)setGifWithMBProgress:(NSString *)string toView:(UIView *)view;
{
     ...
     *  如果修改动画图片就在这里修改
     */
    UIImage *images=[UIImage sd_animatedGIFNamed:@""];
...
+ (void)setNewsWithMBProgress:(NSString *)string toView:(UIView *)view
{
    //提示成功
    MBProgressHUD *newhud  = [[MBProgressHUD alloc] initWithView:view];
    newhud.userInteractionEnabled = YES;//关闭交互
...
+ (void)showAutoDismisWithText:(NSString *)text  view:(UIView *)view
{
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
    // 快速显示一个提示信息
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view animated:YES];
